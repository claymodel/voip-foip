...
make_integer (quad_t i)
{
	...
	if (vp == NULL) {
		cw_log(LOG_WARNING, "malloc() failed\n");
		return(NULL);
	}
...
to_integer (struct val *vp)
{
	...
	if (vp == NULL) {
		cw_log(LOG_WARNING,"vp==NULL in to_integer()\n");
		return(0);
	}
	...
	if (errno != 0) {
		cw_log(LOG_WARNING,"Conversion of %s to integer under/overflowed!\n", vp->u.s);
		free(vp->u.s);
		vp->u.s = 0;
		return(0);
	}
...
op_eq (struct val *a, struct val *b)
{
	...
	if (isstring (a) || isstring (b)) {
		...
		(void)to_integer(b);
#ifdef DEBUG_FOR_CONVERSIONS
		cw_log(LOG_WARNING,"%s to '%lld' and '%lld'\n", buffer, a->u.i, b->u.i);
#endif
...
op_plus (struct val *a, struct val *b)
{
	...
	if (!to_integer (a)) {
		if( !extra_error_message_supplied )
			cw_log(LOG_WARNING,"non-numeric argument\n");
		if (!to_integer (b)) {
			free_value(a);
			free_value(b);
...
op_div (struct val *a, struct val *b)
{
	...
	if (b->u.i == 0) {
		cw_log(LOG_WARNING, "division by zero\n");		
		free_value(a);
		free_value(b);
		return make_integer(INT_MAX);
	}
...
op_rem (struct val *a, struct val *b)
{
	...
	if (b->u.i == 0) {
		cw_log(LOG_WARNING, "div by zero\n");
		free_value(a);
		return(b);
	}
...
op_colon (struct val *a, struct val *b)
{
	...
	if ((eval = regcomp (&rp, b->u.s, REG_EXTENDED)) != 0) {
		...
		cw_log(LOG_WARNING,"regcomp() error : %s",errbuf);
		free_value(a);
		free_value(b);
		return make_str("");		
	}
